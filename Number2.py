...
#Student name: Jacob Sturges, Zachary Knepp
#Date: 4/10/22
...
def evaluate(x):
    ...
    if totalWeight > maxWeight:
       raise ValueError("Weight exceeded")
...
def initial_solution():
    x = np.zeros(n, dtype=int)   #i recommend creating the solution as a list
    ones = myPRNG.randint(0,30)
    while np.dot(x, np.array(weights)) > maxWeight:
       for i in range(ones):
           x[myPRNG.randint(0,n)] = 1
...
while done == 0:
    ...
    for s in Neighborhood:                #evaluate every member in the neighborhood of x_curr
        ...
        try:
            evaluated_no_infeasible = evaluate(s)   # check to see if the solution is feasible
        except:
            continue                    # if solution is infeasible, we will not return the value
        
        if evaluated_no_infeasible[0] > f_best[0]: 
            x_best = s[:]                 #find the best member and keep track of that solution
            f_best = evaluated_no_infeasible[:]        #and store its evaluation  
    
    if f_best == list(f_curr):               #if there were no improving solutions in the neighborhood
        done = 1
    else:
